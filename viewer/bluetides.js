...
(function(ns) {
    ...
    Seadragon.Config.imagePath = "seadragon/img/";
    Seadragon.Config.zoomPerclick = 2.0;
    Seadragon.Config.maxZoomPixelRatio = 10.0;
 ...
 *         type shall be json or html*/
        var source = sprintf("%03d/groups.json", snapid);
        /* hack XXX*/
        ...
        .done(
            function(data) { 
                ...
                for(i = 0; i < data.length; i ++) {
                    if((data[i].center.x > bounds.x)
                    && (data[i].center.x < bounds.x + bounds.width)
                    && (data[i].center.y > bounds.y)
                    && (data[i].center.y < bounds.y + bounds.height)) {
                        console.log(data[i], bounds);
                        data2.push(data[i]);
                    }
                } 
                console.log(data2);
                callback(data2); monitor.done(); }
         )
        .error(
            function(a, b, c) {
                monitor.failed();
